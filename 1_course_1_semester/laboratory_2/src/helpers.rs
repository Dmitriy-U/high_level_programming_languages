...
pub fn get_input_user_data() -> (i32, i32) {
    let user_input = loop {
        let mut user_input = String::new();
        println!("Enter students.txt count and debt separated by whitespace. Example: 5 1:");
        io::stdin().read_line(&mut user_input).expect("An incorrect students.txt count and debt separated was entered. Sample: 5 1");
        user_input = String::from(user_input.trim());
        user_input.replace("  ", " ");
        ...
        if validate_input_string_by_sequence_of_digits(&user_input) {
            ...
            println!("You need to type students.txt count and debt");
        }
    };
